...
meta_bbknn <- read.csv("data/output/embryo_Hb/BatchTest_BBKNN_metadata.csv", header=TRUE, row.names = 1, sep=",", dec=".")
umap_bbknn <- read.csv("data/output/embryo_Hb/BatchTest_BBKNN_umap.csv", header=TRUE, row.names = 1, sep=",", dec=".")
#meta data and t-SNE coordinates after ComBat batch effect correction and downstream clustering in SCANPY
meta_combat <- read.csv("data/output/embryo_Hb/BatchTest_ComBat_metadata.csv", header=TRUE, row.names = 1, sep=",", dec=".")
tsne_combat <- read.csv("data/output/embryo_Hb/BatchTest_ComBat_tsne.csv", header=TRUE, row.names = 1, sep=",", dec=".")
#meta data and t-SNE coordinates before batch effect correction. downstream clustering is performed in SCANPY
meta_raw <- read.csv("data/output/embryo_Hb/BatchTest_raw_metadata.csv", header=TRUE, row.names = 1, sep=",", dec=".")
tsne_raw <- read.csv("data/output/embryo_Hb/BatchTest_raw_tsne.csv", header=TRUE, row.names = 1, sep=",", dec=".")
